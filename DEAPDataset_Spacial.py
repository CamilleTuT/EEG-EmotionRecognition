...
def visualize_window(window):
    ...
    for i, chunk in enumerate(eeg_mean):
        ...
        im, _ = mne.viz.plot_topomap(chunk,
                                     electrodes.positions_2d,
                                     names=electrodes.channel_names,
                                     axes=ax,
                                     cmap='bwr ',
                                     show=False)
...
class DEAPDataset(InMemoryDataset):
    ...
    def __init__(self, root, raw_dir, processed_dir, participant_from, participant_to=None, include_edge_attr=True,
                 undirected_graphs=True, transform=None, pre_transform=None, pre_filter=None, window_size=None):
        self._root = root
        ...
        super(DEAPDataset, self).__init__(root, transform, pre_transform, pre_filter)
        self.load(self.processed_paths[0])
        # self.data, self.slices = torch.load(self.processed_paths[0])

    @property
    def raw_dir(self):
        return f'{self._root}/{self._raw_dir}'

    @property
    def processed_dir(self):
        return f'{self._root}/{self._processed_dir}'
    ...
    def processed_file_names(self):
        ...
        return [f'deap_processed_graph.{file_name}.pt']
    ...
    def process(self):
        ...
        for participant_id in pbar:
            ...
            for index_video, node_features in enumerate(signal_data):
                ...
                edge_attr, edge_index = torch.FloatTensor(edge_attr), torch.tensor([source_nodes1, target_nodes1], dtype=torch.long)
