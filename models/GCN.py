...
class GNNLSTM(torch.nn.Module):
    def __init__(self, input_dim, hidden_channels, target, num_layers=2):
        ...
        self.gconv2 = GraphConv(in_channels=5000, out_channels=3000, aggr='add')
        self.gconv3 = GraphConv(in_channels=3000, out_channels=1000, aggr='add')
        self.lstm = nn.LSTM(2, 3, 2, bidirectional=True)
        # self.mlp = Sequential(Linear(15000, 16),ReLU(), ,ReLU())
        self.mlp = Sequential(Linear(3000, 16), ReLU(), Linear(16, 1))
